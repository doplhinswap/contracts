...
contract DolphinSale {
    ...
    uint256 public startDate = 1624104000;                  // Sat Jun 19 2021 12:00:00 UTC
    uint256 public endDate = 1624406400;                    // Wed Jun 23 2021 00:00:00 UTC
    
    uint256 public totalTokensToSell = 8 * 10**22;          // 80000 DLPH tokens for sell
    uint256 public DLPHPerBnb = 4 * 10**20;                 // 1 BNB = 400 DLPH
    uint256 public minPerTransaction = 0 * 10**18;         // min amount per transaction
    uint256 public maxPerUser = 9 * 10**22;                // max amount per user
    uint256 public totalSold;

    bool public saleEnded;
    
    mapping(address => uint256) public DLPHPerAddresses;
    ...
    // Function to buy DLPH using BNB token
    function buyWithBNB(uint256 buyAmount) public payable checkSaleRequirements(buyAmount) {
        ...
        uint256 sumSoFar = DLPHPerAddresses[msg.sender].add(buyAmount);
        require(sumSoFar <= maxPerUser, 'Greater than the maximum purchase limit');

        DLPHPerAddresses[msg.sender] = sumSoFar;
        totalSold = totalSold.add(buyAmount);
        
        DLPH.transfer(msg.sender, buyAmount);
    ...
    // function to set the total tokens to sell
    // only owner can call this function
    function setTokenPricePerBNB(uint256 _DLPHPerBnb) public {
        require(msg.sender == owner);
        require(_DLPHPerBnb > 0, "Invalid DLPH price per BNB");
        DLPHPerBnb = _DLPHPerBnb;
    }
    ...
    function withdrawUnsoldTokens() public {
        ...
        DLPH.transfer(owner, remainedTokens);
    }
    ...
    function unsoldTokens() public view returns (uint256) {
        ...
        return DLPH.balanceOf(address(this));
    }

    //function to calculate the quantity of DLPH token based on the DLPH price of bnbAmount
    function calculateDLPHAmount(uint256 bnbAmount) public view returns (uint256) {
        uint256 DLPHAmount = DLPHPerBnb.mul(bnbAmount).div(10**18);
        return DLPHAmount;
    }

    //function to calculate the quantity of bnb needed using its DLPH price to buy `buyAmount` of DLPH tokens.
    function calculateBNBAmount(uint256 DLPHAmount) public view returns (uint256) {
        require(DLPHPerBnb > 0, "DLPH price per BNB should be greater than 0");
        uint256 bnbAmount = DLPHAmount.mul(10**18).div(DLPHPerBnb);
        return bnbAmount;
    }
}