...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    // The DLPH TOKEN!
    DolphinToken public DLPH;
    // Dev address.
    address public devAddr;
    // DLPH tokens created per block.
    ...
    // Minimum emission rate: 0.5 DLPH per block.
    ...
    constructor(
        ...
        devAddr = _devAddr;
        DLPH = _DLPH;
        DLPHPerBlock = _initialEmissionRate;
    ...
    function pendingDLPH(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 DLPHReward = multiplier
                .mul(DLPHPerBlock)
                .mul(pool.allocPoint)
                .div(totalAllocPoint);
            accDLPHPerShare = accDLPHPerShare.add(
                DLPHReward.mul(1e12).div(lpSupply)
            );
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        DLPH.mint(address(this), DLPHReward);
        pool.accDLPHPerShare =
            pool.accDLPHPerShare
                .add(DLPHReward
                .mul(1e12)
                .div(lpSupply));
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        ...
        if (referrer != address(0) && referrer != _user && refBonusBP > 0) {
            ...
            DLPH.mint(referrer, refBonusEarned);
